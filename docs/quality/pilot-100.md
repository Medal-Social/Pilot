...
| P100-008 | coverage | CLI and kit plugin coverage did not meet the Pilot 100 target across statements, functions, and lines. | Add behavioral edge-case tests, remove a redundant migration guard, and raise coverage thresholds. | fixed | `pnpm test -- --run --coverage` reports 100% statements/functions/lines for CLI and kit. | Fixed in PR |
