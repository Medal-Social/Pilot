...
| P100-009 | Vitest migration and coverage | Vitest 5 no longer discovers the parent config for CLI tests; the previous inherited coverage scope matched no CLI files and reported an empty success. | Give CLI tests explicit shared settings and CLI source coverage, align runner/provider versions, and cover the newly visible gaps without changing thresholds or exclusions. | fixed | `pnpm quality:100` passes; CLI source coverage: 99.85% statements, 98.09% branches, 100% functions/lines. | Fixed in PR |
