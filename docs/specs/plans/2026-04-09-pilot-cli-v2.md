...
```json
{
  ...
  "private": true,
  "scripts": {
    "dev": "pnpm --filter @medalsocial/pilot-cli dev",
    "build": "pnpm -r build",
    "test": "vitest",
    "lint": "biome check .",
...
```json
{
  "name": "@medalsocial/pilot-plugin-kit",
  "version": "0.1.0",
  "type": "module",
  "main": "dist/index.js",
  "scripts": {
...
{
  "name": "@medalsocial/pilot-plugin-sanity",
  "version": "0.1.0",
  "type": "module",
  "main": "dist/index.js",
...
{
  "name": "@medalsocial/pilot-plugin-pencil",
  "version": "0.1.0",
  "type": "module",
  "main": "dist/index.js",
  "scripts": { "build": "tsc", "test": "vitest" }
...
### Task 39: Proxy support

**Files:**
- Modify: `packages/cli/package.json` — add `proxy-agent` dependency
- Create: `packages/cli/src/lib/network.ts`
- Modify: `packages/cli/src/ai/client.ts` — use proxy-aware fetch
- Modify: `packages/cli/src/ai/offline.ts` — use proxy-aware fetch
- Test: `packages/cli/src/lib/network.test.ts`

```bash
cd packages/cli && pnpm add proxy-agent
```

```ts
// packages/cli/src/lib/network.ts
import { ProxyAgent } from 'proxy-agent';

// ProxyAgent automatically reads HTTP_PROXY, HTTPS_PROXY, NO_PROXY env vars
...
AI client and offline check use `proxyFetch` instead of raw `fetch`. Corporate users behind proxies get automatic support.

---

### Task 40: NO_COLOR / FORCE_COLOR support
...
### Task 42: Switch build to tsup (dual CJS/ESM + DTS)
...
### Task 43: Changesets for multi-package versioning
...
// .changeset/config.json
{
  ...
  "fixed": [],
  "linked": [
    ["@medalsocial/pilot-cli", "@medalsocial/pilot-plugin-kit", "@medalsocial/pilot-plugin-sanity", "@medalsocial/pilot-plugin-pencil"]
  ],
  "access": "restricted",
  "baseBranch": "main",
...
### Task 44: GitHub Actions release pipeline
...
| Proxy support (HTTP_PROXY/HTTPS_PROXY) | Task 39 |
| NO_COLOR / FORCE_COLOR support | Task 40 |
| Shell completions (bash, zsh, fish) | Task 41 |
| tsup dual CJS/ESM build + publint | Task 42 |
| Changesets multi-package versioning | Task 43 |
| GitHub Actions CI + release pipeline | Task 44 |

**Total: 44 tasks across 14 phases.**
